...
def test_selectors_cover_ranges_and_paragraphs_over_sentence_units(tmp_path):
    plan = _plan(tmp_path)
    assert resolve_unit_selection(plan, "all").unit_indices == tuple(unit.index for unit in plan.units)
    ...
    assert resolve_unit_selection(plan, "paragraph:1-2").unit_indices == tuple(unit.index for unit in plan.units)
