...
def _synthesis_status(project: Project) -> dict[str, Any]:
    ...
    try:
        ...
        if reusable == len(plan.units):
            return {"stage": "synthesis", "state": "current", "reason": "current", "reusable": reusable, "total": len(plan.units)}
        return {"stage": "synthesis", "state": "stale", "reason": "synthesis.stale.content_changed", "reusable": reusable, "total": len(plan.units)}
...
def project_status(project: Project) -> dict[str, Any]:
    ...
    synthesis = _synthesis_status(project) if plan_current else {"stage": "synthesis", "state": "stale", "reason": "synthesis.stale.plan_changed"}
    ...
    if state_path.is_file() and project.paths["composition_master"].is_file():
        try:
            ...
            if hash_file(project.paths["composition_master"]) == state.get("master_sha256"):
                composition = {"stage": "composition", "state": "current", "reason": "current", "composition_id": state.get("composition_id")}
    ...
    if output_state.is_file():
        try:
            ...
            if path.is_file() and hash_file(path) == state.get("output_sha256"):
                output = {"stage": "output", "state": "current", "reason": "current", "format": state.get("audio_format")}
        except (OSError, KeyError, ValueError):
            pass
    return {"project": str(project.root), "stages": [*rows, synthesis, composition, output]}


def render_project(project: Project, cfg: Any, *, audio_format: str = "wav", args: Any = None, target_lufs: float | None = None) -> dict[str, Any]:
    ...
    operations.append({"stage": "synthesis", "action": "rebuilt" if synthesis["rendered"] else "skipped", "reused": synthesis["reused"], "rendered": synthesis["rendered"]})
    _, identity = build_audio_job(project, target_lufs=target_lufs)
    state = read_json(project.paths["composition_state"]) if project.paths["composition_state"].is_file() else {}
    if state.get("composition_id") == identity["composition_id"] and project.paths["composition_master"].is_file():
    ...
    expected = {"audio_format": audio_format, "master_sha256": hash_file(project.paths["composition_master"])}
    old = read_json(output_state) if output_state.is_file() else {}
    if old.get("audio_format") == expected["audio_format"] and old.get("master_sha256") == expected["master_sha256"] and output_path.is_file() and hash_file(output_path) == old.get("output_sha256"):
...
def preview_project(
    ...
    *,
    ...
    synthesis = synthesize_project(project, cfg, request=request, selector=selector, activate=activate)
    result = compose_artifacts(synthesis["artifacts"], target_lufs=target_lufs, output=output)
    return {"profile_id": synthesis["profile"].profile_id, "reused": synthesis["reused"], "rendered": synthesis["rendered"], "activated": activate, **result}
