# Readio Standalone SSMD Guide: Dramatic Story or Audiobook SSMD

## Mission

Create immersive narrative fiction or adapt supplied prose into a spoken dramatic reading with controlled pacing and character voices.
...
Use no explicit voice for pure narration. Use `narrator`, `host`, `guest`, or `analyst` only for essential narration or recurring dialogue, and do not force every character to have a distinct voice dependency.

## Recommended structure

1. Establish place, desire, and tension quickly.
2. Build scenes around concrete action rather than exposition alone.
3. Alternate narration and dialogue where useful.
4. Escalate toward a choice, discovery, or confrontation.
5. Use a brief silence before the major reveal.
6. Resolve the scene while leaving an emotional afterimage.

## Use-case writing and performance rules

- Keep narration vivid but speakable.
- Give each recurring character a consistent rhythm rather than extreme voice effects.
- Use `rate="slow"` or `pitch="low"` for select ominous passages only.
- Avoid excessive adverbs in dialogue tags; the TTS delivery can carry some tone.
- Do not insert visual-only formatting.
- For chapter-length output, add `@chapter_one`, `@chapter_two`, and so on.
...
---
ssmd_version: '0.9'
title: The light below the station
pause_defaults:
  enabled: true
  sentence: 250ms
  paragraph: 720ms
...
:::{voice="narrator" rate="slow"}
At midnight, the last train left the underground station empty.
Almost empty.
A blue light still moved beneath the closed platform door.
:::
...
:::{voice="narrator"}
They stepped closer.
The light stopped. ...800ms
Then something knocked from the other side.
:::

:::{voice="analyst" pitch="low"}
Do not open that door.
:::

:::{voice="narrator"}
Neither of them had heard the third voice arrive.
:::
```
