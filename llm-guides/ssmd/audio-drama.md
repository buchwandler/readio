# Readio Standalone SSMD Guide: Audio Drama Scene SSMD

## Mission

Create dialogue-forward dramatic scenes designed specifically for audio, where listeners must understand action without visual stage directions.
...
Use the minimum symbolic cast needed for the scene: `narrator` for essential scene information and `host`, `guest`, or `analyst` for recurring characters. Do not add unbound character names as voice references.

## Recommended structure

1. Establish place and immediate objective in one or two spoken lines.
2. Start the conflict quickly.
3. Let dialogue reveal action, relationships, and stakes.
4. Add only the narration needed to make invisible action understandable.
5. Escalate to a decision or revelation.
6. End the scene on a changed situation.

## Use-case writing and performance rules

- Do not rely on visual stage directions.
- Replace “he points at the door” with dialogue or minimal narration that a listener can follow.
- Keep voice turns clear and relatively short during tense exchanges.
- Use pauses for hesitation, interruption, or revelation.
- Avoid embedding external audio files; describe essential sound events in narration unless the user explicitly has a supported audio workflow.
- Do not add unbound character role names to `voice=`.
...
<div voice="narrator">
A lock clicks behind them.
The room has no windows and one metal door.
</div>
...
<div voice="analyst" rate="slow">
Then we have exactly sixty seconds.
</div>
...
<div voice="analyst">
Until the room asks us a question.
</div>

<div voice="narrator">
A light appears above the door. ...700ms
A calm electronic voice begins to count down.
</div>
```
