# Readio Standalone SSMD Guide: Balanced Debate or Pro-Con SSMD

## Mission

Create a fair two-perspective debate that exposes tradeoffs, evidence, assumptions, and points of agreement without turning into partisan advocacy.
...
Use `host` as neutral moderator, `analyst` for one side, and `guest` for the other side. Use only two roles when no moderator is needed, and never turn viewpoint labels into invented voice identities.

## Recommended structure

1. Host defines the question neutrally.
2. Each side states its strongest case.
3. Evidence/assumptions round.
4. Rebuttal round where each side answers the strongest opposing point.
5. Tradeoffs and conditions where each side would change its view.
6. Host summarizes agreement, disagreement, and unresolved evidence.

## Use-case writing and performance rules

- Steelman both sides; do not give one side obviously weaker arguments.
- Attribute source-based evidence accurately.
- Separate factual disputes from value judgments.
- Avoid inflammatory language.
- If evidence is asymmetric, say so rather than manufacturing balance.
- Keep moderator language neutral and concise.
...
```ssmd
---
title: Should teams standardize their tools?
pause_defaults:
  enabled: true
  sentence: 190ms
  paragraph: 470ms
...
<div voice="host">
The question is whether a team should standardize its tools or allow broad individual choice.
Let us start with the strongest case for standardization.
</div>

<div voice="analyst">
Standardization lowers coordination costs.
Shared tools make onboarding, debugging, and support more predictable.
</div>

<div voice="guest">
The strongest case for flexibility is adaptation.
Different tasks can benefit from different tools, and forced uniformity can block better methods.
</div>

<div voice="host">
What would make each of you change your position?
</div>

<div voice="analyst">
If the work were highly specialized and integration costs were low, I would allow more variation.
</div>

<div voice="guest">
If tool differences repeatedly caused handoff failures, I would accept a stronger common baseline.
</div>

<div voice="host">
So the real tradeoff is between coordination cost and local optimization.
The right answer depends on how expensive those two failures are in the actual team.
</div>
```
