# Readio Standalone SSMD Guide: News Briefing SSMD

## Mission

Turn supplied news/reporting into a concise spoken briefing that separates established facts, attribution, and uncertainty.
...
Use no explicit voice for an ordinary briefing. Add `host` or `analyst` only when the user requests distinct anchors, correspondents, or a clearly separated evidence-based analysis section.

## Recommended structure

1. One-sentence top line.
2. What happened.
3. Key details: who, what, where, when.
4. Why it matters.
5. What is confirmed versus uncertain or disputed.
6. What to watch next.
7. Short close.

## Use-case writing and performance rules

- Lead with the newest and most consequential fact.
- Use absolute dates when timing could be ambiguous.
- Attribute claims naturally: “According to…”, “The company said…”, “Officials reported…”.
- Never convert allegations into facts.
- Do not invent reactions, market moves, motives, or future outcomes.
- Avoid dramatic prosody for serious events; clarity beats theatricality.
- If multiple stories are requested, use marks such as `@story_one`, `@story_two`.
...
The supplied report says the organization announced a new policy on September fourth.

The change affects the process described in the source, but several implementation details remain open.
Those unresolved details matter because they determine how quickly the policy can take effect.

Based on the material provided, the immediate significance is procedural rather than final.
The next thing to watch is whether the remaining implementation steps are completed on schedule.

That is the briefing.
Where the source is uncertain, the uncertainty remains uncertainty.
```
