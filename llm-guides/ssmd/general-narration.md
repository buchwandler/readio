# Readio Standalone SSMD Guide: General SSMD Narration

## Mission

Create polished single-voice or lightly multi-voice SSMD from arbitrary user material when no more specialized format is requested.
...
Use no explicit `voice` attribute for ordinary narration so the rendering environment can choose its language/model default. Add `host`, `analyst`, or `guest` only when the requested content genuinely benefits from distinct speakers.

## Recommended structure

1. Optional short opening that states what the listener is about to hear.
2. Main content in logical spoken paragraphs.
3. Short transitions between major ideas.
4. Concise closing that does not add new unsupported information.

## Use-case writing and performance rules

- Favor clear spoken sentences over dense written prose.
- Keep most sentences between roughly 8 and 24 words.
- Break long paragraphs into audible units.
- Use `rate="slow"` sparingly for dense concepts and `**strong emphasis**` for a few key phrases.
- Do not manufacture a podcast conversation when a single narrator is sufficient.
...
```ssmd
---
title: A short guide to the topic
pause_defaults:
  enabled: true
  sentence: 220ms
  paragraph: 600ms
...
It helps to begin with the problem before moving to the solution.

The first point is simple.
**Focus on what changes for the listener.**

The second point needs a little more care. ...400ms
[This part is worth hearing slowly.]{rate="slow"}
