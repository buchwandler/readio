# Readio Standalone SSMD Guide: Solo Podcast SSMD

## Mission

Turn a topic, article, notes, or source material into an engaging one-host podcast episode without fake dialogue.
...
Use one implicit renderer-selected voice for the episode. Add `host` only when the requested document explicitly needs a semantic host role or a second voice is introduced.

## Recommended structure

1. Cold open: one intriguing claim, question, or tension point.
2. Welcome and episode promise.
3. Two to five clearly transitioned segments.
4. Mid-episode recap for longer episodes.
5. Takeaways.
6. Natural sign-off.

## Use-case writing and performance rules

- Write conversationally, but do not fill space with fake banter.
- Use rhetorical questions occasionally.
- Let the host explain why each point matters before moving on.
- For episodes longer than about eight minutes, add `@intro`, `@topic_one`, `@topic_two`, and `@conclusion` marks.
- Do not claim personal experiences for the host unless the user supplied them.
...
@intro
A tiny default can quietly shape thousands of decisions.
That is what we are unpacking today.

Welcome.
In the next few minutes, we will look at why defaults matter, where they help, and where they can mislead us.

@topic_one
Start with the simplest case.
A good default removes needless work without taking away meaningful choice.

But there is a catch. ...450ms
**Convenience can become invisible influence.**

@conclusion
So the practical takeaway is straightforward.
Choose defaults deliberately, make alternatives visible, and review them when the context changes.
