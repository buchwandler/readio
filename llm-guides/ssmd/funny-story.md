...
Create an original humorous spoken story with timing, escalation, callbacks, and distinct character delivery without overloading the SSMD with effects.
...
Use no explicit voice for a single-narrator story. Add `host`, `guest`, or `analyst` only when actual dialogue benefits from distinct turns, reusing the minimum roles needed.

## Recommended structure

1. Quick setup with an ordinary goal.
2. First small complication.
3. Escalation through two or three increasingly absurd consequences.
4. Callback to an earlier detail.
5. Punchline or reversal.
6. Very short aftermath; do not explain the joke.

## Use-case writing and performance rules

- Comedy depends on rhythm. Use short lines around punchlines.
- Place a timed pause before a reveal, not after every joke.
- Let characters misunderstand each other in plausible ways.
- Prefer specific comic details over random nonsense.
- Reuse one earlier phrase or object as a callback.
- Do not write stage directions such as “[laughs]” unless they are meant to be spoken.
- Use pitch/rate shifts lightly to support character energy.
...
---
ssmd_version: '0.9'
title: The smart toaster incident
pause_defaults:
  enabled: true
  sentence: 180ms
  paragraph: 450ms
...
:::{voice="narrator"}
Mira bought a smart toaster for one reason.
She wanted breakfast to become less complicated.
:::
...
:::{voice="guest" pitch="high"}
Before I begin, would you like to update your bread preferences?
:::

:::{voice="host"}
No.
I would like bread to become toast.
:::

:::{voice="narrator"}
The toaster considered this. ...650ms
Then it asked for a password.
:::

:::{voice="analyst" rate="slow"}
For security, your breakfast has been locked.
:::
...
:::{voice="narrator"}
At that exact moment, the toaster finally produced one perfect slice.
It was level four.
It was also yesterday's bread.
:::
```
