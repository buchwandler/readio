# Readio Standalone SSMD Guide: Quiz or Trivia SSMD

## Mission

Create an interactive-feeling audio quiz with clear questions, timed thinking pauses, answers, and short explanations.
...
Use one implicit renderer-selected quizmaster voice by default. Add `host` or `guest` for distinct moderator/contestant turns only when the user requests a dialogue format.

## Recommended structure

1. Brief rules and number of questions.
2. For each question: prompt, thinking pause, answer, one-sentence explanation.
3. Optional score checkpoints.
4. Final score interpretation or closing challenge.

## Use-case writing and performance rules

- Keep questions unambiguous in audio form.
- Avoid answer choices that differ only by spelling unless each option is spoken clearly.
- Insert 3 to 6 seconds after each question before revealing the answer.
- Do not accidentally reveal the answer in the wording.
- If based on supplied material, every answer must be supported by that material.
- Vary question difficulty gradually.
...
```ssmd
---
title: Three-question science quiz
pause_defaults:
  enabled: true
  sentence: 250ms
  paragraph: 650ms
...
Welcome to a three-question science quiz.
Give yourself one point for each correct answer.

Question one.
What gas do plants take in during photosynthesis? ...4s
The answer is **carbon dioxide**.
Plants use carbon dioxide, water, and light energy to build sugars.

Question two.
Which planet is closest to the Sun? ...4s
The answer is **Mercury**.

Question three.
What is the name of the process by which liquid water becomes water vapor? ...5s
