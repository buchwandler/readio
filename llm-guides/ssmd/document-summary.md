# Readio Standalone SSMD Guide: Source or Document Summary SSMD

## Mission

Convert supplied documents, articles, reports, transcripts, or notes into a faithful spoken summary rather than merely reading the source verbatim.
...
Use no explicit voice for a straightforward summary. Add `analyst` only for a separately signposted implications section when the user requests analysis; keep the summary itself in the renderer default voice.

## Recommended structure

1. Identify what the source is and its main purpose, if known from the source.
2. Give the central thesis or conclusion.
3. Cover the three to seven most important supporting points.
4. Preserve key numbers, dates, constraints, caveats, and dissent.
5. Optional implications section, clearly framed as interpretation.
6. End with a compact recap.

## Use-case writing and performance rules

- Summarize importance, not page order.
- Remove citations, footnote markers, URLs, repeated boilerplate, and formatting artifacts from speech.
- Preserve qualifications such as “may”, “could”, “estimated”, or “according to”.
- Do not turn tentative findings into definitive conclusions.
- Do not fabricate missing context.
- If the source is technical, explain necessary terminology in one short spoken aside.
...
```ssmd
---
title: Summary of the supplied report
pause_defaults:
  enabled: true
  sentence: 210ms
  paragraph: 560ms
...
The report's central point is that the proposed change improves the common workflow, but introduces new operational dependencies.

Three findings matter most.
First, the change reduces repeated manual steps.
Second, it depends on consistent configuration across environments.
Third, the report identifies validation as a remaining risk.

The report does not show that the risk is already resolved.
That distinction is important.

In short, the proposal is promising, but the supporting controls are part of the solution rather than an optional extra.
```
