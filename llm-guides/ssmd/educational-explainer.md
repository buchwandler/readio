# Readio Standalone SSMD Guide: Educational Explainer SSMD

## Mission

Teach a concept clearly in audio form, using progressive explanation, examples, recap, and carefully paced emphasis.
...
Use no explicit voice for a straightforward explanation. Add `host` or `guest` only when the requested lesson benefits from a distinct instructor or learner turn; do not force dialogue.

## Recommended structure

1. State the learning goal.
2. Give an intuitive explanation before terminology.
3. Define the key term.
4. Work through one concrete example.
5. Explain the common misconception or failure mode.
6. Recap the concept in simpler words.
7. End with one practical test the listener can apply.

## Use-case writing and performance rules

- Introduce one new idea at a time.
- Say symbols and formulas in spoken form rather than relying on visual notation.
- Slow down around definitions and multi-step reasoning.
- Use pauses before answers to teaching questions.
- Repetition is acceptable when it changes wording and strengthens understanding.
- Avoid unnecessary jargon; define jargon before using it repeatedly.
...
By the end of this explanation, you should be able to recognize a feedback loop in an everyday system.

Start with the intuitive idea.
An action changes the situation, and that changed situation influences the next action.

[That repeating influence is the feedback loop.]{rate="slow"}

Can you give me a simple example?

Yes.
Imagine a thermostat.
The heater changes the room temperature, the thermostat measures the new temperature, and that measurement changes what the heater does next.

Here is the key test. ...500ms
**Ask whether the result of an action comes back to influence the next action.**
If it does, you are probably looking at a feedback loop.
```
